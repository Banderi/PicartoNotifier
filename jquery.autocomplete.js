...
(function ($, window, document, undefined) {
  ...
  EmailAutocomplete.prototype = {
    init: function () {
      ...
      //wrap our field
      var $wrap = $("<div class='eac-input-wrap' />");
	  // .css({
      ...
      this.$suggOverlay = $("<span class='"+this.options.suggClass+"' />")
	  // .css({
        // display: "block",
        // "box-sizing": "content-box", //standardize
        // lineHeight: this.$field.css('lineHeight'),
        // paddingTop: heightPad - 2 + "px",
    ...
    suggest: function (str) {
      ...
      var match = this._domains.filter(function (domain) {
        return 0 === domain.toLowerCase().indexOf(str.toLowerCase());
      }).shift() || "";
		
		// if (match.replace(str, "") !== match) {
      // return match.replace(str, "");
	  // } else if (match.replace(str.toLowerCase(), "") !== match) {
      // return match.replace(str.toLowerCase(), "");
	  // }
	  
	  return match.replace(RegExp(str, "i"), "");
	  },
